...
public class Utilities {
    ...
    public static byte[] addIPHead(IExtensionHelpers helpers, IHttpRequestResponse requestResponse,String key) {
    	...
    	map.put("Referer","Referer: https://localhost.com/");
    	map.put("Cookie","Cookie: rememberMe=1");
    ...
    public static byte[] ContentType(IExtensionHelpers helpers, IHttpRequestResponse requestResponse,String uaid) {
    	Map<String,String> map = new HashMap<String,String>();
        map.put("jpg", "image/jpeg");
        map.put("XHTML", "application/xhtml+xml ");
        map.put("JSON", "application/json");
        map.put("streamdata", "application/octet-stream");
        map.put("formurlencoded", "application/x-www-form-urlencoded");
        map.put("formdata", "multipart/form-data ");
    	byte[] request = requestResponse.getRequest();
        IRequestInfo requestInfo = helpers.analyzeRequest(request);
        int bodyOffset = requestInfo.getBodyOffset();
        String body = new String(request, bodyOffset, request.length - bodyOffset);
        ...
        for(int i = 0;i<headers.size();i++){
        	if(headers.get(i).startsWith("ContentType")){
        		String strUA = String.format("ContentType: %s", map.get(uaid));
        		headers.set(i, strUA); 
        		isExists = true;
        	}
        }
        if(!isExists){
        	headers.add(String.format("ContentType: %s", map.get(uaid)));
        }
